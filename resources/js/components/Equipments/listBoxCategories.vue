<template>
    <div class="col-span-6 sm:col-span-6">
        <Listbox as="div" v-model="selected" @click.prevent="handleSelection()">
            <ListboxLabel class="block text-sm font-medium text-gray-700">Categoria</ListboxLabel>
            <div class="relative mt-1 rounded-md shadow-sm">
                <ListboxButton :class="[
                    ...
                    <span class="block truncate">{{ selected?.categoria || 'Seleccione una categoría' }}</span>
                ...
                <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
                    ...
                    <ListboxOptions
                        ...
                        <ListboxOption as="template" v-for="category in list" :key="category.clvCategoria" :value="category"
            ...
            <div v-if="errorsList">
                <ul>
                    <li class="mt-2 text-sm text-red-600" id="categoria-error" v-for="error in errorsList" :key="error">{{
                        error }}</li>
                </ul>
            </div>
        </Listbox>
...
<script>
export default {
    name: 'listBoxCategoriesComponent',
}
</script>

@props(['text', 'action', 'SelectTypeCategory'])
...
<div id="create-modal-category" name="create-modal-category" class="hidden">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Agregar Categoria
                        </h3>
...
@push('scripts')
    @if (request()->is('Dashboard/Panel/Equipments'))
        <script>
            ...
            document.addEventListener('DOMContentLoadedCreateCategory', function() {

                showModalButtonCreateCategory.addEventListener('click', function() {
                    createModalCategory.classList.remove('hidden');
                    window.location.hash = "createModalCategory";
                    nombreCategoriaInput.focus();
                });

                document.getElementById('background').addEventListener('click', function() {
                    document.getElementById('create-modal-category').classList.add('hidden');
                    window.location.hash = "";
                });

                document.getElementById('btn-create-modal-category-close').addEventListener('click', function() {
                    document.getElementById('create-modal-category').classList.add('hidden');
                    window.location.hash = "";
                });

                if (window.location.hash === "#createModalCategory") {
                    document.getElementById("create-modal-category").classList.remove("hidden");
                    nombreCategoriaInput.focus();
                }
            });

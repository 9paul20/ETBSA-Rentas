<div class="col-span-6 sm:col-span-6">
    ...
    <select id="clvTipoCategoria" name="clvTipoCategoria"
        class="mt-1 block w-full rounded-md border-gray-300 py-2 pl-3 pr-10 text-base focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm @error('clvTipoCategoria') border-red-400 @enderror"
        required>
        @if (count($Data['allTypeCategories']) > 0)
            ...
            @foreach ($Data['allTypeCategories'] as $rowTypeCategory)
                <option value="{{ $rowTypeCategory->clvTipoCategoria }}"
                    @if ($clvTipoCategoria == $rowTypeCategory->clvTipoCategoria) selected @endif>

...
<div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
    <button id="btn-create-modal-fixedExpensesCatalogs" type="button"
...
<div id="create-modal-fixedExpensesCatalogs" name="create-modal-fixedExpensesCatalogs" class="hidden">
                ...
                <div
                    ...
                    <form action="{{ $action }}" class="mt-4 space-y-4" method="PUT">
                        ...
                        <div class="col-span-6 sm:col-span-6">
                            ...
                            <input type="text" name="gastoFijo" id="gastoFijo" autocomplete="given-gastoFijo"
                        ...
                        <div class="mt-5 sm:mt-6 flex justify-end space-x-2">
                            ...
                            <button type="button" id="btn-create-modal-fixedExpensesCatalogs-close"
...
@push('scripts')
    @if (request()->is('Dashboard/Panel/FixedExpenses'))
        <script>
            var showModalButtonCreateFixedExpensesCatalogs = document.getElementById('btn-create-modal-fixedExpensesCatalogs');
            var createModalFixedExpensesCatalogs = document.getElementById('create-modal-fixedExpensesCatalogs');
            var nombreGastoFijoInput = document.getElementById('gastoFijo');
            // var modalContent = document.querySelector(
            //     '.flex.min-h-full.items-end.justify-center.p-4.text-center.sm\\:items-center.sm\\:p-0');
            document.addEventListener('DOMContentLoadedCreateFixedExpensesCatalogs', function() {

                showModalButtonCreateFixedExpensesCatalogs.addEventListener('click', function() {
                    createModalFixedExpensesCatalogs.classList.remove('hidden');
                    window.location.hash = "createModalFixedExpensesCatalogs";
                    nombreGastoFijoInput.focus();
                });

                document.getElementById('background').addEventListener('click', function() {
                    document.getElementById('create-modal-fixedExpensesCatalogs').classList.add('hidden');
                    window.location.hash = "";
                });

                document.getElementById('btn-create-modal-fixedExpensesCatalogs-close').addEventListener('click',
                    function() {
                        document.getElementById('create-modal-fixedExpensesCatalogs').classList.add('hidden');
                        window.location.hash = "";
                    });

                if (window.location.hash === "#createModalFixedExpensesCatalogs") {
                    document.getElementById("create-modal-fixedExpensesCatalogs").classList.remove("hidden");
                    nombreGastoFijoInput.focus();
                }
            });
            var DOMContentLoadedCreateFixedExpensesCatalogs = new Event('DOMContentLoadedCreateFixedExpensesCatalogs');
            document.dispatchEvent(DOMContentLoadedCreateFixedExpensesCatalogs);
        </script>
    @endif
@endpush

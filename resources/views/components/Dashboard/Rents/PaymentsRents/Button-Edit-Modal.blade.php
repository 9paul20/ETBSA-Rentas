@props(['id', 'pagoRenta', 'ivaRenta', 'descripcion', 'href', 'SelectStatusPaymentRent'])
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Editar Pago De Renta
                        </h3>
                    ...
                    <form action="{{ $href }}" class="mt-4 space-y-4" method="POST" id="edit-form-{{ $id }}">
                        ...
                        <div class="col-span-6 sm:col-span-6">
                            <label for="pagoRenta" class="block text-sm font-medium text-gray-700">PagoRenta</label>
                            <input type="number" name="pagoRenta" id="pagoRenta-{{ $id }}"
                                autocomplete="given-pagoRenta"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('pagoRenta') border-red-400 @enderror"
                                value="{{ $pagoRenta }}" required autofocus step="0.01">
                            @error('pagoRenta')
                            <div class="flex
                        ...
                        <div class="col-span-6 sm:col-span-6">
                            <label for="ivaRenta" class="block text-sm font-medium text-gray-700">IvaRenta</label>
                            <input type="number" name="ivaRenta" id="ivaRenta-{{ $id }}" autocomplete="given-ivaRenta"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('ivaRenta') border-red-400 @enderror"
                                value="{{ $ivaRenta }}" required autofocus step="0.01">
                            @error('ivaRenta')
                            <div class="flex
                                    items-center mt-1 text-red-400">
                                <i class="fas fa-exclamation-triangle mr-2"></i>
...
<script>
    var editModal{{ $id }} = document.getElementById('edit-modal-{{ $id }}');
            var nombrePagoRentaInput{{ $id }} = document.getElementById(
                'pagoRenta-{{ $id }}');

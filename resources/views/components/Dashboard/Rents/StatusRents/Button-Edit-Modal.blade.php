@props(['id', 'estadoRenta', 'textColor', 'bgColorPrimary', 'bgColorSecondary', 'descripcion', 'href'])
                    ...
                    <form action="{{ $href }}" class="mt-4 space-y-4" method="POST" id="edit-form-{{ $id }}">
                        ...
                        <div class="col-span-6 sm:col-span-6">
                            <label for="estadoRenta" class="block text-sm font-medium text-gray-700">Estado De
                                Renta</label>
                            <input type="text" name="estadoRenta" id="estadoRenta-{{ $id }}"
                                autocomplete="given-estadoRenta"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('estadoRenta') border-red-400 @enderror"
                                value="{{ $estadoRenta }}" required autofocus>
                            @error('estadoRenta')
                            <div class="flex
...
<script>
    var editModal{{ $id }} = document.getElementById('edit-modal-{{ $id }}');
            var nombreEstadoRentaInput{{ $id }} = document.getElementById(
                'estadoRenta-{{ $id }}');
            ...
            function openModal{{ $id }}() {
                ...
                nombreEstadoRentaInput{{ $id }}.focus();
            }

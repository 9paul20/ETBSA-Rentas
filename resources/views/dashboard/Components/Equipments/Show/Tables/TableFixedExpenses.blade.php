        ...
        <div class="mx-auto max-w-7xl">
            @if (count($Data['tableFixedExpenses']['rowFixedExpenses']) > 0)
                <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
                                ...
                                @foreach ($Data['tableFixedExpenses']['rowFixedExpenses'] as $rowFixedExpense)
                                    <tr class="hover:bg-gray-100">
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            <div class="text-gray-600">{{ $rowFixedExpense['descripcion'] }}</div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    ...
                    {{ $Data['tableFixedExpenses']['rowFixedExpenses']->appends(['fixedExpenses_page' => $Data['tableFixedExpenses']['rowFixedExpenses']->currentPage()])->links('vendor.pagination.tailwind') }}
                </div>
            @else

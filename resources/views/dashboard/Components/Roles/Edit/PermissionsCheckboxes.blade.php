...
<div class="relative flex items-start">
    <div class="flex h-5 items-center">
        <input id="permissions{{ $permission->name }}" aria-describedby="{{ $permission->name }}-description"
            ...
            class="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500" {{ $role->permissions instanceof
        \Illuminate\Support\Collection && $role->permissions->contains($permission->id) ? 'checked' : '' }}>
    </div>
    <div class="ml-3 text-sm">
        <label for="permissions{{ $permission->name }}" class="font-medium text-gray-700">{{ $permission->display_name
            }} ->
